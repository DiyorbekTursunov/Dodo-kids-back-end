...
export const getConsolidatedCaseTrackerStatus = async (req: Request, res: Response) => {
  try {
    ...
    const productPacks: RawProductPack[] = await prisma.productPack.findMany({
      include: {
        Product: {
          select: {
            id: true,
    ...
    const formattedPacks: FormattedProductPack[] = productPacks.map(pack => {
      ...
      if (pack.status.length > 0) {
        sendedCount = pack.status.reduce((sum, process) => sum + process.sendedCount, 0);
        acceptCount = pack.status.reduce((sum, process) => sum + process.acceptCount, 0);
        residueCount = pack.status.reduce((sum, process) => sum + process.residueCount, 0);
      }
