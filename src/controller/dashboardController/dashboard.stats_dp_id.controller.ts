...
export const getEmployeeStats = async (req: Request, res: Response) => {
  try {
    ...
    const employeeProcesses = await prisma.productProtsess.findMany({
      ...
      select: {
        invoiceId: true, // Fixed: Changed productpackId to invoiceId
      },
      distinct: ["invoiceId"],
    });

    // Get the IDs of all Invoices handled by this employee
    const invoiceIds = employeeProcesses.map(
      (process) => process.invoiceId
    );

    // Get all Invoices this employee has worked on
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        totalProductCount,
        productPackCount: invoices.length, // Note: Represents invoice count
        stats: {
        ...
        productPacks: invoices, // Note: Renamed internally to invoices but kept response key for compatibility
        processes,
      },
    });
  } catch (error) {
